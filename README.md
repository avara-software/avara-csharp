...
The Avara C# SDK provides convenient access to the [Avara REST API](https://docs.avarasoftware.com) from applications written in C#.

It is generated with [Stainless](https://www.stainless.com/).

The REST API documentation can be found on [docs.avarasoftware.com](https://docs.avarasoftware.com).

## Installation

```bash
git clone git@github.com:stainless-sdks/avara-csharp.git
...
var study = await client.Viewer.Studies.Create(parameters);
...
// Configured using the AVARA_API_KEY, AVARA_WEBHOOK_KEY and AVARA_BASE_URL environment variables
AvaraClient client = new();
```
...
AvaraClient client = new() { ApiKey = "My API Key" };
```
...
| ------------ | -------------------- | -------- | --------------------------------- |
| `ApiKey`     | `AVARA_API_KEY`      | true     | -                                 |
| `WebhookKey` | `AVARA_WEBHOOK_KEY`  | false    | -                                 |
| `BaseUrl`    | `AVARA_BASE_URL`     | true     | `"https://api.avarasoftware.com"` |
...
var study = await client
    ...
    .Viewer.Studies.Create(parameters);
...
To send a request to the Avara API, build an instance of some `Params` class and pass it to the corresponding client method. When the response is received, it will be deserialized into an instance of a C# class.

For example, `client.Viewer.Studies.Create` should be called with an instance of `StudyCreateParams`, and it will return an instance of `Task<StudyCreateResponse>`.
...
```csharp
var response = await client.WithRawResponse.Viewer.Studies.Create(parameters);
...
var response = await client.WithRawResponse.Viewer.Studies.Create(parameters);
StudyCreateResponse deserialized = await response.Deserialize();
...
- `AvaraApiException`: Base class for API errors. See this table for which exception subclass is thrown for each HTTP status code:

| Status | Exception                            |
| ------ | ------------------------------------ |
| 400    | `AvaraBadRequestException`           |
| 401    | `AvaraUnauthorizedException`         |
| 403    | `AvaraForbiddenException`            |
| 404    | `AvaraNotFoundException`             |
| 422    | `AvaraUnprocessableEntityException`  |
| 429    | `AvaraRateLimitException`            |
| 5xx    | `Avara5xxException`                  |
| others | `AvaraUnexpectedStatusCodeException` |

Additionally, all 4xx errors inherit from `Avara4xxException`.

- `AvaraIOException`: I/O networking errors.

- `AvaraInvalidDataException`: Failure to interpret successfully parsed data. For example, when accessing a property that's supposed to be required, but the API unexpectedly omitted it from the response.

- `AvaraException`: Base class for all exceptions.
...
AvaraClient client = new() { MaxRetries = 3 };
```
...
AvaraClient client = new() { Timeout = TimeSpan.FromSeconds(42) };
```
...
AvaraClient client = new() { HttpClient = httpClient };
```
...
var parameters = StudyCreateParams.FromRawUnchecked
(
...
var response = client.Viewer.Studies.Create(parameters)
...
By default, the SDK will not throw an exception in this case. It will throw `AvaraInvalidDataException` only if you directly access the property.

If you would prefer to check that the response is completely well-typed upfront, then either call `Validate`:

```csharp
var study = client.Viewer.Studies.Create(parameters);
study.Validate();
```
...
AvaraClient client = new() { ResponseValidation = true };
```
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/avara-csharp/issues) with questions, bugs, or suggestions.
