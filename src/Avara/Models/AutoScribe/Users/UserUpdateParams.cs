...
/// <summary>
/// Updates a user's profile information, permissions, and AutoScribe-specific settings.
/// All fields are optional - only provided fields will be updated. Email cannot
/// be changed via API. NPI number is required if enabling report creation capability.
///
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/autoScribe/users/{0}", this.UserID)
        )
        {
