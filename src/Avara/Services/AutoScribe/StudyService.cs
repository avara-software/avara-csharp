...
public sealed class StudyService : IStudyService
{
    ...
    /// <inheritdoc/>
    public async Task<StudyViewerOnlyRerouteUrlResponse> ViewerOnlyRerouteUrl(
        StudyViewerOnlyRerouteUrlParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        using var response = await this
            .WithRawResponse.ViewerOnlyRerouteUrl(parameters, cancellationToken)
            .ConfigureAwait(false);
        return await response.Deserialize(cancellationToken).ConfigureAwait(false);
    }
}
...
public sealed class StudyServiceWithRawResponse : IStudyServiceWithRawResponse
{
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<StudyViewerOnlyRerouteUrlResponse>> ViewerOnlyRerouteUrl(
        StudyViewerOnlyRerouteUrlParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        HttpRequest<StudyViewerOnlyRerouteUrlParams> request = new()
        {
            Method = HttpMethod.Post,
            Params = parameters,
        };
        ...
        return new(
            ...
            {
                var deserializedResponse = await response
                    .Deserialize<StudyViewerOnlyRerouteUrlResponse>(token)
                    .ConfigureAwait(false);
                if (this._client.ResponseValidation)
                {
                    deserializedResponse.Validate();
