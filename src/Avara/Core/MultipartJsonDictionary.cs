...
sealed class MultipartJsonDictionary
{
        ...
        where T : class
    {
        ...
        {
            throw new AvaraInvalidDataException($"'{key}' cannot be absent");
        }
        T deserialized;
        try
        {
            deserialized =
                ...
                ?? throw new AvaraInvalidDataException($"'{key}' cannot be null");
        }
        catch (JsonException e)
        {
            throw new AvaraInvalidDataException($"'{key}' must be of type {typeof(T).FullName}", e);
        }
